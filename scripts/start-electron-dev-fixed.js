...
async function startElectronDev() {
  ...
  // FIX: Usar npm.cmd en Windows sin shell
  ...
  const frontend = spawn(npmCmd, ['run', 'dev:frontend'], {
    ...
    cwd: rootDir,
    detached: true,
    windowsHide: true,  // Solo funciona sin shell: true
  ...
  // FIX: Usar npx.cmd en Windows sin shell
  const npxCmd = isWindows ? 'npx.cmd' : 'npx';
  
  const electron = spawn(npxCmd, ['electron', 'electron/main.js'], {
