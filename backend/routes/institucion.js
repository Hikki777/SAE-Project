...
router.post('/init', upload.fields([{ name: 'logo', maxCount: 1 }, { name: 'admin_foto', maxCount: 1 }]), [
  ...
  try {
    ...
    // Generar Master Recovery Key (12 chars alfanuméricos)
    const masterRecoveryKey = Math.random().toString(36).substring(2, 8).toUpperCase() + Math.random().toString(36).substring(2, 8).toUpperCase();
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const institucionData = {
        ...
        master_recovery_key: masterRecoveryKey,
        inicializado: true
      };
      ...
      res.json({ success: true, institucion, masterRecoveryKey });
    });
